...
class JSecurImage {
	...
	*/
	...
	public function __construct()
	{
		...
		$this->wordlist_file = JPATH_ADMINISTRATOR.'/components/com_rsmembership/helpers/securimage/words/words.txt';
		$this->use_wordlist  = false;

		$this->gd_font_file  = JPATH_ADMINISTRATOR.'/components/com_rsmembership/helpers/securimage/gdfonts/automatic.gdf';
		$this->use_gd_font   = false;
		$this->gd_font_size  = 24;
		$this->text_x_start  = 15;

		$this->ttf_file      = JPATH_ADMINISTRATOR.'/components/com_rsmembership/helpers/securimage/AHGBold.ttf';
		...
		$this->signature_font  = JPATH_ADMINISTRATOR.'/components/com_rsmembership/helpers/securimage/AHGBold.ttf';

		$this->audio_path   = JPATH_ADMINISTRATOR.'/components/com_rsmembership/helpers/securimage/audio/';
	 ...
	 */
	private function getBackgroundFromDirectory()
	{
		$images = array();
	 ...
	 */
	private function readCodeFromFile()
	{
	...
	private function saveData()
	{
		...
		$session->set('com_rsmembership.securimage.ctime', $date->toUnix());
	}
	...
	{
		$session = JFactory::getSession();
		return $session->get('com_rsmembership.securimage.value', '');
	}
	 ...
	 */
	private function generateWAV($letters)
	{
		$data_len    = 0;
		$files       = array();
		$out_data    = '';
	 ...
	 */
	private function isCodeExpired()
	{
		$session = JFactory::getSession();
		$creation_time = $session->get('com_rsmembership.securimage.ctime', 0);
		$expired = true;
		$date = JFactory::getDate();
		$current_date = $date->toUnix();
		
		if (!is_numeric($this->expiry_time) || $this->expiry_time < 1)
			$expired = false;
		else if ( $current_date - $creation_time < $this->expiry_time ) 
			$expired = false;
