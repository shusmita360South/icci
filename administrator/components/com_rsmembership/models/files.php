...
class RSMembershipModelFiles extends JModelList
{
	...
	public function remove($cids) {
		...
		if ($files) {
			...
			$query->delete()
				  ->from($db->qn('#__rsmembership_files'))
				  ->where($db->qn('id').' IN ('.RSMembershipHelper::quoteImplode(array_keys($files)).')');
			$db->setQuery($query);
			$db->execute();
		}
	...
	public function addsubscriberfiles($files)
	{
		$db 	= JFactory::getDBO();
		$query	= $db->getQuery(true);
	...
	public function addmembershipfolders($folders)
	{
		$db 			= JFactory::getDBO();
		$query			= $db->getQuery(true);
		$membership_id  = JFactory::getApplication()->input->get('membership_id', 0, 'int');
	...
	public function addextravaluefolders($folders)
	{
		$db 	= JFactory::getDBO();
		$query	= $db->getQuery(true);
