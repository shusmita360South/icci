...
class RSMembershipModelMembership extends JModelAdmin
{
	...
	public function getAttachmentsPagination()
	{
	...
	public function save($data) 
	{
		...
		if ( parent::save($data) ) 
		{
			...
			$extras = array_map('intval', $extras);
		...
		{
			JFactory::getApplication()->enqueueMessage($this->getError(), 'warning');
			return false;
		}
	}
	...
	public function delete(&$cids)
	{
		$db		 = JFactory::getDBO();
		$query	 = $db->getQuery(true);
		$in_cids = RSMembershipHelper::quoteImplode($cids);
	...
	public function foldersPublish($cid=array(), $publish=1)
	{
		...
		if (!is_array($cid) || count($cid) > 0)
		{
			...
			$query->update($db->qn('#__rsmembership_membership_shared'))->set($db->qn('published').' = '.$db->q($publish))->where($db->qn('id').' IN ('.RSMembershipHelper::quoteImplode($cid).')');
			$db->setQuery($query);
	...
	public function foldersRemove($cids) 
	{
		...
		$query->delete()->from($db->qn('#__rsmembership_membership_shared'))->where($db->qn('id').' IN ('.RSMembershipHelper::quoteImplode($cids).')');
		$db->setQuery($query);
		$db->execute();
	...
	*/
	// Attachment - Publish
	public function attachmentsPublish($cid=array(), $publish=1)
	{
		$db 	= JFactory::getDBO();
		$query	= $db->getQuery(true);
		...
		if (!is_array($cid) || count($cid) > 0) 
		{
			...
			$query->clear();
			$query->update($db->qn('#__rsmembership_membership_attachments'))->set($db->qn('published').' = '.$db->q($publish))->where($db->qn('id').' IN ('.RSMembershipHelper::quoteImplode($cid).')');
			$db->setQuery($query);
	...
	// Attachment - Remove
	public function attachmentsRemove($cids)
	{
		$db 	= JFactory::getDBO();
		$query	= $db->getQuery(true);

		$query->delete()->from($db->qn('#__rsmembership_membership_attachments'))->where($db->qn('id').' IN ('.RSMembershipHelper::quoteImplode($cids).')');
		$db->setQuery($query);
		$db->execute();
