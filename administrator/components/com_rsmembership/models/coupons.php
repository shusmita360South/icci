...
class RSMembershipModelCoupons extends JModelList
{
	public function __construct($config = array())
	{
		if (empty($config['filter_fields'])) 
			$config['filter_fields'] = array('id', 'name', 'published', 'date_start', 'date_end', 'discount_type, discount_price');
	...
	protected function getListQuery()
	{
		...
		$listOrdering  	= $this->getState('list.ordering', 'date_added');
	...
	protected function populateState($ordering = null, $direction = null) 
	{
		...
		$this->setState($this->context.'.filter.search', 		 $app->getUserStateFromRequest($this->context.'.coupons.search', 'filter_search'));
		$this->setState($this->context.'.filter.filter_state', 	 $app->getUserStateFromRequest($this->context.'.coupons.filter_state', 'filter_state'));

		parent::populateState('date_added', 'DESC');
	}
	...
	public function getFilterBar() 
	{
		...
		$options['listOrder']  = $this->getState('list.ordering', 'date_added');
		...
		$options['sortFields'] = array(
			...
			JHtml::_('select.option', 'date_added', 	JText::_('COM_RSMEMBERSHIP_DATE_ADDED')),
			JHtml::_('select.option', 'name', 			JText::_('COM_RSMEMBERSHIP_COUPON_CODE')),
			JHtml::_('select.option', 'date_start', 	JText::_('COM_RSMEMBERSHIP_FROM')),
			JHtml::_('select.option', 'date_end', 		JText::_('COM_RSMEMBERSHIP_TO')),
			JHtml::_('select.option', 'discount_type, discount_price', 	JText::_('COM_RSMEMBERSHIP_DISCOUNT')),
