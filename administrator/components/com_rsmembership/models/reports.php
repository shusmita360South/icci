	...
	public function getReportData($post_filters) 
	{
		...
		if( $report == 'report_2' || $report == 'report_3' ) 
		{
			...
			if ( !empty($transaction_types) ) {
				$query->where($db->qn('type') . ' IN ('.RSMembershipHelper::quoteImplode($transaction_types).')');
			}

			$gateways = ( isset($post_filters['gateways']) ? $post_filters['gateways'] : array() );
			if ( !empty($gateways) ) {
				$query->where($db->qn('gateway') . ' IN ('.RSMembershipHelper::quoteImplode($gateways).')');
			}
			...
			if ( !empty($status) ) {
				...
				$query->where( $db->qn('status') . ' IN ('.RSMembershipHelper::quoteImplode($status).')' );
			}
		}
		else 
		{
			if ( !empty($memberships) ) 
				$query->where( $db->qn('membership_id') . ' IN ('.RSMembershipHelper::quoteImplode($memberships).')' );

			$status = isset($post_filters['status_memberships']) ? $post_filters['status_memberships'] : false;
			if ( !empty($status) ) 
				$query->where( $db->qn('status') . ' IN ('.RSMembershipHelper::quoteImplode($status).')' );
		}
