...
class RSMembershipModelTerms extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->
			select('*')->
			from($db->qn('#__rsmembership_terms'));
	...
	protected function populateState($ordering = null, $direction = null) 
	{
		...
		$this->setState($this->context.'.filter.search', 		 $app->getUserStateFromRequest($this->context.'.terms.search', 'filter_search'));
		$this->setState($this->context.'.filter.filter_state', 	 $app->getUserStateFromRequest($this->context.'.terms.filter_state', 'filter_state'));
	...
	public function getFilterBar() 
	{
		...
		$options['sortFields'] = array(
			...
			JHtml::_('select.option', 'name', 		JText::_('COM_RSMEMBERSHIP_TERM')),
