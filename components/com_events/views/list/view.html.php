 ...
 */
class EventsViewList extends JViewLegacy
{
	protected $items;
	...
	protected function _prepareDocument()
	{
		...
		if($menu)
		{
			...
			$this->params->def('page_heading', JText::_('com_events_DEFAULT_PAGE_TITLE'));
		}
	...
	public function getCurrentSearch($array2 = array())
	{
		...
		return JRoute::_('index.php?option=com_events&' . http_build_query($array1, '', '&amp;'));
	}
	...
	public static function getTypes() {
		...
		$query = $db->getQuery(true);
		$query->select('*')
		    ->from($db->quoteName('#__events_categories'))
		    ->order('ordering DESC');
		$db->setQuery($query);
		$authors = $db->loadObjectList();
