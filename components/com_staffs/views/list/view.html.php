 ...
 */
class StaffsViewList extends JViewLegacy
{
	protected $items;
	...
	public function display($tpl = null)
	{
        ...
        $this->staffs		= $this->get('Staffs');
        $this->pagination	= $this->get('Pagination');
        $this->params       = $app->getParams('com_staffs');
	...
	public function getCurrentSearch($array2 = array())
	{
		...
		return JRoute::_('index.php?option=com_staffs&' . http_build_query($array1, '', '&amp;'));
	}
	...
	public static function getTypes() {
		...
		$query = $db->getQuery(true);
		$query->select('*')
		    ->from($db->quoteName('#__staffs_categories'))
		    ->order('ordering DESC');
		$db->setQuery($query);
		$authors = $db->loadObjectList();
