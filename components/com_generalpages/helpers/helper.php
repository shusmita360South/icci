...
abstract class GeneralpagesHelper
{
	...
	public static function getCategory($id)
	{
		...
		$query->select('*');
		$query->from('#__generalpages_categories');
		$query->where('id = '.$id);
	...
	public static function getType($id)
	{
		...
		$query->select('*');
		$query->from('#__generalpages_types');
		$query->where('id = '.$id);
...
#		echo '<pre>'; print_r($rows); echo '</pre>';
	...
	public static function getRelatedGeneralpages() {
		$db  = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query->select('*')
		    ->from($db->quoteName('#__generalpages_items'))
		    ->where($db->quoteName('state') . ' = 1')
		    ->order('ordering ASC');
		$db->setQuery($query);
		$generalpages = $db->loadObjectList();
